  ...
  render() {
    ...
    if(Object.keys(this.props.weather).length === 0) {
      ...
      daysListResult = this.state.daysPlaceholder.map((dayObject) => {
        return (
           <li className={"weather-panel__day" + ` ${dayObject.classText}` + (this.state.inTransition ? " in-transition" : "")} key={dayObject.dayTitle}>
      ...
      daysListResult = this.props.weather.days.map((day) => {
        return (
           <li className={"weather-panel__day" + ` ${day.day}` + (this.state.inTransition ? " in-transition" : "")} key={day.day}>
